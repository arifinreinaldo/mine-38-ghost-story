...
// The three pillars the player must assemble to convict (deduction step).
// Each slot accepts any evidence id in its list as a correct proof.
export const SOLUTION = {
  killer: "dimas",
  motive: ["wa"],                 // the threat that gave Dimas a reason
  means: ["termos", "headlamp"],  // drugged coffee + swapped battery
  opportunity: ["posisi"],        // only Dimas was behind Arya
};

// `icon` keys map to inline SVGs in components/Icons.jsx
// `locked: true` evidence stays hidden until an interrogation reveals it.
...
// Per-suspect rebuttal shown when the player accuses the wrong person.
...
// Detective ranks (best to worst), used on the Reveal screen.
export const RANKS = {
  utama: { label: "Detektif Utama", note: "Pelaku tepat, pembuktian sempurna, dan kau membongkar jebakannya." },
  inspektur: { label: "Inspektur", note: "Pelaku tepat dan pembuktian kuat." },
